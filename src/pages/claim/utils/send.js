...
async function sendCoinTx(toAddress, chain, amount) {
    try {
        ...
        const msg = SendMsg(wallet[1], toAddress, amount * MicroFactor, currentChain.denom);
        const response = await Transaction(wallet[0], wallet[1], [msg], txFee, "", currentChain.rpc);
